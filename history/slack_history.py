...
def getHistory(pageableObject, channelId, pageSize = 100):
    ...
    while(True):
        ...
        try:
            ...
            # TODO: expand this to timeout condition; can i just re-issue the pageableObject.history() call?
            print("Something went wrong! E:{0}".format(e))

        messages.extend(response['messages'])

        if (response['has_more'] == True):
            lastTimestamp = messages[-1]['ts'] # -1 means last element in a list
            print("Sleeping a tick to avoid hitting slack's rate limits...")
            # This might be OK at 1.  Had god results downloading several years of DMs at a value of 2
            time.sleep(2)
        else:
            break
    return messages
...
def writeMessageFile( fileName, messages ):
    try:
        ...
        # I don't know why this happen from time to time; i think it's due to channel names
        # that might be duplicates or the API is mashing them into duplicate names
        # we try to resecue and still save the content, though
        print("not found: {0}! messages:{1}".format(fileName, messages))
        fp = os.path.basename(fileName)
        with open(fp, 'w') as oops:
            json.dump( messages, oops, indent=4)
...
def getChannels(slack, dryRun):
    ...
    if not dryRun:
        ...
        for channel in channels:
            ...
            saved_names = []
            for i in args.names_only.split(','):
                saved_names.append(i)

            channelDir = channel['name']
            if channelDir in saved_names:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest='names_only',
        default=False,
        help="names channel, private channel, directMessages to be saved only")
